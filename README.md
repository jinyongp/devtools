...
See the [management roadmap](docs/management-roadmap.md) for implementation scope
and recorded validation results.
...
| Goal | Guide |
| --- | --- |
| Install or update a binary | [Installation](docs/install.md) |
| Connect a project, register values, import dotenv, run commands | [CLI usage](docs/cli-contract.md) |
| Diagnose tools and required values | [Doctor](docs/doctor.md) |
| Allocate ports and connect local projects | [Ports](docs/ports.md) |
| Start a background server and wait for it | [Processes](docs/processes.md), [readiness](docs/process-readiness.md) |
| Plan work, claim tasks, recover a session | [Tasks and workstreams](docs/tasks.md) |
| Manage profiles in the browser | [Dashboard API and UI](docs/management-api-contract.md) |
| Back up, restore, or reclaim storage | [Backup](docs/backup.md), [cleanup](docs/cleanup.md) |
| Verify installed binaries in isolation | [Sandbox](docker/README.md) |
...
The following table covers the base CLI and value commands. Task, port, process,
backup, and cleanup guides describe their domain errors. `schema` provides
command inputs and output shapes; branch on the returned `error.code`.
