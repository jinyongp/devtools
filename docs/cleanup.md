...
예시의 `PLAN_ID`는 preview 응답의 `data.id`, `ITEM_ID`는 `data.items`에서
선택한 항목의 `id`다. `ARCHIVE_ID`는 archives 응답에서 선택한 보관함 ID다.
`UUID`는 새 변경 요청마다 생성하고, 같은 apply를 재시도할 때 유지한다.
