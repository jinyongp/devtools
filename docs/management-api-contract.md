...
변경은 도구별 허용 action과 구조화된 입력으로 전달한다. 대상 profile과 입력을 명시하고, 변경 종류에 따라 요청 UUID·편집 리비전·실행 ID·미리 보기 ID로 정확한 대상을 고정한다. HTTP 계층은 도구의 공통 검증·변경 로직을 호출한다. task 관리 변경은 요청 ID와 최신 profile 리비전으로 검사한다.
