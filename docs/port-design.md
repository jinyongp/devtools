...
할당 객체의 공통 필드는 `profile`, `instance_id`, `alias`, `directory`,
`name`, `port`, `location_status`다. location_status는 `available`, `missing`,
`unknown`으로 실행 위치의 접근 상태를 나타낸다. alias가 없으면 null이다.
show는 `data.item`, list는 `data.items`에 할당 객체를 반환한다.
allocate는 `data.item`에 할당, `data.changed`에 신규 할당 여부를 반환한다.
고정된 host나 URL 대신 bind 템플릿으로 접속 문자열을 구성한다.

check는 `data.item`의 할당 필드에 `occupancy`(`free`, `in_use`, `unknown`)와
`tcp_reachable`(boolean 또는 확인 불가 시 null)을 추가한다. 진단 응답은
정상 수행 시 종료 코드 0이며, 점유만으로 서비스 소유자를 확정하지 않는다.
release는 `data.changed`를 반환하고 이미 해제된 서비스는 false다.
prune은 `data.removed`와 사유 코드가 포함된 `data.retained` instance 배열,
그리고 실제 제거 여부인 `data.changed`를 반환한다.
활성 run이 있는 할당은 OS 포트가 아직 비어 있어도 release·prune에서 보류한다.

instance 객체는 `profile`, `instance_id`, `alias`, `directory`, `location_status`를 사용한다.
list는 `data.items`, show는 `data.item`, name·move는 `data.item`과 `data.changed`,
remove는 `data.changed`를 반환한다. 같은 상태로의 수정이나 이미 제거한 대상의
재제거는 changed가 false다. 모호한 대상 선택은 오류로 처리한다.
공통 응답과 이전 JSON 경로의 변경은 [CLI 출력 계약](cli-output.md)을 따른다.
