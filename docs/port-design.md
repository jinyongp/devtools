...
| `strict` | true이면 지정한 port를 사용하고 충돌 시 오류 | `false` |
...
strict가 true이면 port가 필수이며 해당 포트만 사용한다. 함께 선언한
range는 자동 선택용 설정으로 유지한다. strict를 false로 변경하면 적용된다.
자동 선택 범위의 후보를 모두 사용 중이면 할당 오류를 반환한다.

기존 할당이 현재 설정을 만족하고 유효하면 우선 재사용한다. strict가 true이면
지정한 port와 같아야 하고, false이면 지정한 port 또는 적용 범위 안의 값이어야 한다.
...
참조의 필드명, 전역 기본 범위의 값과 설정 방법, 실행 인자 참조 문법이다.
