...
func taskBody(def tasks.Definition) map[string]any {
	...
	for _, field := range def.Fields {
		...
		case "compaction_fingerprint":
			s = map[string]any{"type": "string", "pattern": "^[0-9a-f]{64}$"}
		case "compaction_through":
			s = map[string]any{"type": "string", "pattern": positiveIntegerPattern}
		default:
